...
- January 24, 2025. Amélioration interface utilisateur formulaires : documentation d'aide complète ajoutée avec liens officiels pour chaque plateforme de réseaux sociaux, correction débordement tooltips avec largeur maximale (max-w-[200px]) et taille réduite (text-xs), repositionnement icônes d'aide à droite des labels pour meilleure ergonomie, section "Documentation officielle" avec liens externes sécurisés pour faciliter configuration des tokens API
- January 29, 2025. Optimisation génération calendrier éditorial : implémentation barre de progression réaliste sur 1 heure avec simulation continue et polling intelligent, amélioration gestion d'erreur webhook n8n avec messages spécifiques (workflow en mode test, non activé, timeout), correction timeout étendu à 1 heure maximum au lieu de 15 minutes, affichage progressif des minutes écoulées sur total pour meilleure expérience utilisateur
- January 29, 2025. Interface d'aide n8n complète : ajout bouton d'aide (?) dans dialogue génération calendrier éditorial avec instructions détaillées pour activation workflow n8n, distinction claire entre mode production (Activate) et mode test (Execute Workflow), messages d'erreur améliorés avec instructions précises pour résoudre problème de webhook non disponible, guide visuel avec étapes numérotées et exemples de code
- January 29, 2025. Génération en arrière-plan sans attente webhook : modification système pour lancer webhook n8n en arrière-plan sans attendre sa réponse immédiate, évite erreurs d'affichage si workflow non activé, génération continue automatiquement pendant 1 heure avec polling intelligent pour détecter nouveaux contenus, messages informatifs sur génération en cours plutôt qu'erreurs bloquantes
- January 29, 2025. Progression fixe 1 heure sans détection automatique : suppression du système de détection automatique des contenus générés, implémentation barre de progression qui dure exactement 60 minutes sans interruption même si du contenu est détecté, fonction simulateProgressFor1Hour remplace le polling intelligent, respect strict de la durée d'1 heure comme demandé par l'utilisateur
- January 29, 2025. Interface discrète sans indication de durée : suppression de l'affichage "(X/60 min)" pour que l'utilisateur ne sache pas que la génération dure 1 heure, message générique "Génération de contenu par IA en cours..." sans référence temporelle, interface volontairement opaque sur la durée réelle du processus
